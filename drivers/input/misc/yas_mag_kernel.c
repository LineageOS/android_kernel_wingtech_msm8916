...
static enum hrtimer_restart yas_timer_func(struct hrtimer *timer)
{
	...
	hrtimer_forward_now(&st->poll_timer, ns_to_ktime(st->delay[MAG_DATA_FLAG]));
...
#if CTS_TEST
		if (st->use_hrtimer) {
			hrtimer_start(&st->poll_timer, ns_to_ktime(0), HRTIMER_MODE_REL);
		} else {
...
static int yas_probe(struct i2c_client *i2c, const struct i2c_device_id *id)
{
	...
	if (st->use_hrtimer) {
		hrtimer_init(&st->poll_timer, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
